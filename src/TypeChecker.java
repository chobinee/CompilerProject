...
public class TypeChecker {
    ...
    static Type Check(For f, TypeEnv te) {
    	addType(f.decls, te); //선언문
    	Type t = Check(f.expr, te); //조건식
    	Type assignment = Check(f.asmt, te); //대입문
    	Type ts = Check(f.stmts, te); //stmt들의 type check
    	if(t == Type.BOOL)
    		if(ts == Type.VOID)
    			f.type = ts;
    		else
