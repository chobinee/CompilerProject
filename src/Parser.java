...
public class Parser {
    ...
    private For forStmt() {
    	...
    	Expr expr = expr();
    	match(Token.SEMICOLON);
    	Stmt asmt = assignment(); //assignment 끌어다 사용하여 뒤에 세미콜론이 붙음.
    	match(Token.RPAREN);
    	Stmts stmt = stmts();
    	match(Token.END); //stmt가 여러개 올 수 있으므로 end 토큰을 통해 for문이 끝남을 알게 함.
    	return new For(decls, expr, (Assignment) asmt, stmt);
